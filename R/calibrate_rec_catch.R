...
calibrate_rec_catch <- function(p_star_cod, p_star_had, select_mode, k, select_month,
  ...
  if (length(MRIP_data$dtrip) == 0) {
    pds_new_all <- data.frame(period2 = NA, tot_keep_cod = 0, tot_keep_had = 0,
                              tot_rel_cod = 0, tot_rel_had = 0, tot_cod_catch = 0, tot_had_catch = 0,
                              estimated_trips = 0, n_choice_occasions = 0, n_cal_draw = k, mode = select_mode, month = select_month)
    costs_new_all <-  data.frame(tripid = NA, cost = NA, catch_draw = NA,  tot_keep_cod_base = NA, tot_rel_cod_base = NA, age = NA,
                                 days_fished = NA, beta_opt_out_age = NA, beta_opt_out_likely = NA, beta_opt_out_prefer = NA, tot_keep_had_base = NA, tot_rel_had_base = NA,
                                 beta_cost = NA, beta_opt_out = NA, beta_sqrt_hadd_keep = NA, beta_sqrt_hadd_release = NA, beta_sqrt_cod_hadd_keep = NA, beta_sqrt_cod_keep = NA,
                                 beta_sqrt_cod_release = NA, period2 = NA, n_cal_draw = k, mode = select_mode, month = select_month)
    ...
    pstar<- read.csv(file.path(here::here(paste0(pstar_file_path, "/pstar_", select_mode,"_", select_month, ".csv")))) %>%
