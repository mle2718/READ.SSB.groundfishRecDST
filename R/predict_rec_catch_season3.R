...
#Catch at length draws are 1-100
CaL_draw<- baseline_comparison$draw_id

#indicate whether we need keep to release, or release to keep, for both species 
...
calibration_data_table = feather::read_feather(paste0("C:/Users/andrew.carr-harris/Desktop/cod_hadd_RDM/pds_new_", i,".feather"))

costs_new_all = feather::read_feather(paste0("C:/Users/andrew.carr-harris/Desktop/cod_hadd_RDM/costs_", i,".feather"))
...
cod_size_data <- size_data_read %>% dplyr::filter(species == "cod", season == seas, draw==CaL_draw)
had_size_data <- size_data_read %>% dplyr::filter(species == "hadd", season == seas, draw==CaL_draw)
#Unadjusted data
...
if(cod_catch_check !=0){
  ...
  catch_size_data <- cod_catch_data %>%
    dplyr::mutate(fitted_length = sample(cod_size_data$length,
                                         nrow(.),
                                         prob = cod_size_data$proj_CaL_prob,
...
if (had_catch_check!=0){
  ...
  catch_size_data_had <- had_catch_data %>%
    dplyr::mutate(fitted_length = sample(had_size_data$length,
                                         nrow(.),
                                         prob = had_size_data$proj_CaL_prob,
...
if (cod_catch_check!=0){
  ...
  if (cod_release_2_keep==1){
    ...
    trip_data<-trip_data %>% 
      ...
      dplyr::mutate(tot_keep_cod_new1=ifelse(release_to_keep==1,tot_rel_cod_new+tot_keep_hadd_new, tot_keep_cod_new), 
...
##code for reallocating cod keep as release 
if (cod_release_2_keep==1){
...
check_long<-check_long %>% 
  dplyr::mutate(release = ifelse(species=="cod", 0, count_rel), 
                keep = ifelse(species=="cod", count_keep+count_rel, count_keep)) %>% 
...
if (hadd_release_2_keep==1){
  ...
  check_long<-check_long %>% 
    dplyr::mutate(release = ifelse(species=="had", 0, count_rel), 
                  keep = ifelse(species=="had", count_keep+count_rel, count_keep)) %>% 
    dplyr::select(-count_keep, -count_rel)
...
length_weight<- length_expanded %>%
  ...
  dplyr::mutate(Discmortality_Total_weight = ifelse(keep_release=="release", Discard_mortality * Number_at_Length * weight,0),
...
saveRDS(output2, file = "C:/Users/andrew.carr-harris/Desktop/cod_hadd_RDM/predictions.rds")
write_xlsx(output2, "C:/Users/andrew.carr-harris/Desktop/cod_hadd_RDM/predictions.xlsx")


##This code sums the data over draws. 
# full_output<-output2 %>%
#   dplyr::group_by(Category, catch_disposition, param, number_weight, mrip_index, run) %>%
#   dplyr::summarise(sum_value=sum(Value), .groups="drop") %>%
#   dplyr::filter(((Category=="cod" | Category=="had") & (catch_disposition=="keep" | catch_disposition=="release") &
#                   param=="Total" & number_weight=="Number") | (Category=="ntrips" | number_weight=="n_choice_occasions") | Category=="CV") %>%
#   dplyr::mutate(parameter=case_when(Category=="ntrips"~ "estimated_trips" ,
#                 Category=="nchoiceoccasions"~  "n_choice_occasions",
#                 Category=="cod" & catch_disposition=="keep"~ "tot_keep_cod_model" ,
#                 Category=="cod" & catch_disposition=="release"~ "tot_rel_cod_model" ,
#                 Category=="had" & catch_disposition=="keep"~ "tot_keep_hadd_model" ,
#                 Category=="had" & catch_disposition=="release"~ "tot_rel_hadd_model",
#                 Category=="CV"~"CV")) %>%
#   dplyr::select(mrip_index, run, sum_value, parameter) %>%
#   dplyr::rename(sum_value_proj=sum_value,
#                 draw=run)
#return(predictions)


##I used this code to compare my projections under no chnages to the calibraiton. Not needed for projections. 

# output3<-data.frame() 
# output4<-data.frame() 
# 
# for(i in unique(full_output$mrip_index)){
# 
#   calibration_data_table = feather::read_feather(paste0("C:/Users/andrew.carr-harris/Desktop/cod_hadd_RDM/pds_new_", i,".feather"))
#   calibration_data_table$mrip_index<-i
#   output3<-calibration_data_table
#   
#   output4<-rbind(output4, output3)
# }
# 
# calib_output<-output4 %>% 
#   dplyr::group_by(draw, mrip_index, mode, open) %>% 
#   dplyr::summarise(sum_estimated_trips=sum(estimated_trips),
#                    sum_n_choice_occasions=sum(n_choice_occasions), 
#                    sum_tot_cat_cod_model=sum(tot_cat_cod_model), 
#                    sum_tot_cat_hadd_model=sum(tot_cat_hadd_model), 
#                    sum_tot_keep_cod_model=sum(tot_keep_cod_model), 
#                    sum_tot_keep_hadd_model=sum(tot_keep_hadd_model), 
#                    sum_tot_rel_cod_model=sum(tot_rel_cod_model), 
#                    sum_tot_rel_hadd_model=sum(tot_rel_hadd_model), .groups="drop")
# 
# mode<-calib_output %>% 
#   dplyr::distinct(mode) %>% 
#   dplyr::group_by(mode) %>% 
#   dplyr::mutate(mode_id=dplyr::cur_group_id())
#   
#   
#   calib_output_long <- calib_output %>%
#   tidyr::pivot_longer(cols = c(sum_estimated_trips,sum_n_choice_occasions, sum_tot_cat_cod_model , sum_tot_cat_hadd_model ,
#                                sum_tot_keep_cod_model, sum_tot_keep_hadd_model, sum_tot_rel_cod_model, sum_tot_rel_hadd_model), 
#                       names_to = c("parameter" ), 
#                       names_pattern = "sum_(.*)", 
#                       values_to="sum_value_calib") %>% 
#   dplyr::arrange(draw, parameter, mrip_index) %>% 
#   dplyr::filter(parameter!="tot_cat_hadd_model" & parameter!="tot_cat_cod_model" ) %>% 
#   dplyr::right_join(full_output, by=c("draw", "parameter", "mrip_index")) %>% 
#   dplyr::filter(parameter!="CV")  %>% 
#   dplyr::group_by(mode) %>% 
#   dplyr::mutate(mode_id=dplyr::cur_group_id()) %>% 
#   dplyr::ungroup() %>% 
#   dplyr::group_by(draw, parameter,mode_id, open) %>% 
#   dplyr::summarise(value_calib=sum(sum_value_calib),
#                    value_proj=sum(sum_value_proj), .groups="drop") %>% 
#   replace(is.na(.), 0) %>% 
#   dplyr::mutate(diff=value_calib-value_proj) %>%
#   dplyr::arrange(diff)
#   dplyr::ungroup() %>% 
#   dplyr::group_by(parameter) %>% 
#   dplyr::summarise(mean_diff=mean(diff), 
#                    sd_diff=sd(diff), .groups="drop")