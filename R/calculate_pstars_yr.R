...
select_mode = "fh"
starting_pstar_cod = 0.5
starting_pstar_had = 0.5
k = 1


calculate_pstars_yr<- function(select_mode, starting_pstar_cod, starting_pstar_had){
  ...
  for(k in 1:150){
    ...
    repeat{
      ...
      pstar <- calibrate_pstars_yr( p_star_cod, p_star_had,
      ...
      if(cod_harvest_harv_diff %in% c("NaN", "NA", NA) & had_harvest_harv_diff %in% c("NaN", "NA", NA)){
        ...
        if(cod_harvest_harv_diff %in% c("NaN", "NA", NA)){
          if(had_harvest_harv_diff<0 & abs(had_harvest_harv_diff)>1){
            p_star_had<-p_star_had +.02 # some values smaller step because the code kept finding valleys at high incremental change.
          }

          if(had_harvest_harv_diff>0 & abs(had_harvest_harv_diff)>1){
            p_star_had<-p_star_had -.02
          }
