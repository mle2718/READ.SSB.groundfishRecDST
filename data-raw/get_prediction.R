...
for(k in 1:10){
  for(i in 0:1){
    ...
    predictions<- predict_rec_catch(
      ...
      catch_data_all = read.csv(paste0("C:/Users/kimberly.bastille/Desktop/codhad_data/catch_draws/catch_draws", k, "_full.csv")),
      #l_w_conversion =
...
predictions_out2<- predictions_out %>%
  dplyr::reframe(Value  = mean(as.numeric(Value)), .by = c(mode, Category, keep_release))
...
test_plot2<- predictions_out2 %>%
  dplyr::filter(Category == c("CV", "ntrips"))%>%
  ggplot2::ggplot(ggplot2::aes(x = Category, y = Value , fill=mode))+
  ggplot2::geom_col(position="dodge")#+
  #ggplot2::facet_grid(run)
